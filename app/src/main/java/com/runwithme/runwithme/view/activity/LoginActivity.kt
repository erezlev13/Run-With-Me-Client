...
class LoginActivity : AppCompatActivity() {
    ...
    private fun readLocalUserData() {
        mLoginViewModel.readUser.observeOnce(this, { database ->
            if (database.isNotEmpty()) {
                mLoginViewModel.isValidToken(database[0].token)
                binding.loginProgressBar.show()
                mLoginViewModel.tokenResponse.observeOnce(this, { response ->
                    when (response) {
                        is NetworkResult.Success -> {
                            ...
                            Snackbar.make(
                                binding.loginButton,
                                "Welcome back! We'll connect you in a sec",
                                Snackbar.LENGTH_LONG
                            ).show()
                        }
                    }
    ...
    private fun login() {
        ...
        mLoginViewModel.login(LoginRequest(email, password))
        binding.loginProgressBar.show()
