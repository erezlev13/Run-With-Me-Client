...
class StepDetector {
    ...
    fun updateAccel(timeNs: Long, x: Float, y: Float, z: Float) {
        ...
        val normalization_factor: Float = SensorFilter.norm(worldZ)
        worldZ[0] = worldZ[0] / normalization_factor
        worldZ[1] = worldZ[1] / normalization_factor
        worldZ[2] = worldZ[2] / normalization_factor
        val currentZ: Float = SensorFilter.dot(worldZ, currentAccel) - normalization_factor
        velRingCounter++
        velRing[velRingCounter % VEL_RING_SIZE] = currentZ
        val velocityEstimate: Float = SensorFilter.sum(velRing)
